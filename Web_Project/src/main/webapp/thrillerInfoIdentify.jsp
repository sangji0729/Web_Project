...
<body>
	<h1 style="text-align: center;">Thriller 게시판 작성한 글 조회</h1>
	<hr>
...
<c:when test="${fn:length(list) > 0 }">
		<ul id="ulTable">
			...
			<c:forEach items="${list }" var="l">
				<li>
					<ul>
						<li>${l.tno }</li>
						<li><a href="./thrillerDetail?tno=${l.tno }">
						 <c:choose>
									<c:when test="${l.tthumbnail eq null }">
										<img alt="no image" src="./img/noimage.jpg"
											style="vertical-align: middle; height: 100px;">
									</c:when>
									<c:otherwise>
										<img alt="thumb" src="./thumbnail/${l.tthumbnail }"
											style="vertical-align: middle; height: 100px;">
											</c:otherwise>
								</c:choose>
						</a> <a href="./thrillerDetail?tno=${l.tno }">${l.ttitle } <c:if
									test="${l.commentcount > 0}">
									<small> [ 댓글수 : ${l.commentcount } ]</small>
								</c:if>
								<c:if test="${l.tlike > 0 }">
								<small>[ 추천수 : ${l.tlike } ]</small>
								</c:if>
						</a></li>
						<li>${l.name }</li>
						<li>${l.tdate }</li>
		...
		<c:otherwise>출력할 글이 없습니다.</c:otherwise>
			...
			<div id="paging">
				<!-- 이동할 때 마다 페이지명을 변수처리 -->
				<c:set var="pageName" value="thrillerInfoIdentify" scope="request" />
