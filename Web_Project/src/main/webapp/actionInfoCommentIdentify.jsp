...
<body>
	<h1 style="text-align: center;">Action 게시판 작성한 댓글 조회</h1>
	<hr>
...
<c:when test="${fn:length(list) > 0 }">
		<ul id="ulTable">
			...
			<c:forEach items="${list }" var="l">
				<li>
					<ul>
						<li>${l.acno }</li>
						<li><a href="./actionBoardDetail?ano=${l.ano }">${l.accontent }</a></li>
						<li>${l.ano }</li>
						<li>${l.name }</li>
						<li>${l.acdate }</li>
					</ul>
		...
		<c:otherwise>출력할 댓글이 없습니다.</c:otherwise>
			...
			<div id="paging">
				<!-- 이동할 때 마다 페이지명을 변수처리 -->
				<c:set var="pageName" value="actionInfoCommentIdentify" scope="request" />
