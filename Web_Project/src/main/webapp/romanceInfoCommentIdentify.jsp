...
<body>
	<h1 style="text-align: center;">Romance 게시판 작성한 댓글 조회</h1>
	<hr>
...
<c:when test="${fn:length(list) > 0 }">
		<ul id="ulTable">
			...
			<c:forEach items="${list }" var="l">
				<li>
					<ul>
						<li>${l.rcno }</li>
						<li><a href="./romanceDetail?rno=${l.rno }">${l.rccontent }</a></li>
						<li>${l.rno }</li>
						<li>${l.name }</li>
						<li>${l.rcdate }</li>
					</ul>
		...
		<c:otherwise>출력할 댓글이 없습니다.</c:otherwise>
			...
			<div id="paging">
				<!-- 이동할 때 마다 페이지명을 변수처리 -->
				<c:set var="pageName" value="romanceInfoCommentIdentify" scope="request" />
