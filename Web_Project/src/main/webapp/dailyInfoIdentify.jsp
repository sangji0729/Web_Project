...
<body>
	<h1 style="text-align: center;">Daily 게시판 작성한 글 조회</h1>
	<hr>
...
<c:when test="${fn:length(list) > 0 }">
		<ul id="ulTable">
			...
			<c:forEach items="${list }" var="l">
				<li>
					<ul>
						<li>${l.dano }</li>
						<li><a href="./dailyDetail?dano=${l.dano }">
						 <c:choose>
									<c:when test="${l.dathumbnail eq null }">
										<img alt="no image" src="./img/noimage.jpg"
											style="vertical-align: middle; height: 100px;">
									</c:when>
									<c:otherwise>
										<img alt="thumb" src="./thumbnail/${l.dathumbnail }"
											style="vertical-align: middle; height: 100px;">
											</c:otherwise>
								</c:choose>
						</a> <a href="./dailyDetail?dano=${l.dano }">${l.datitle } <c:if
									test="${l.commentcount > 0}">
									<small> [ 댓글수 : ${l.commentcount } ]</small>
								</c:if>
								<c:if test="${l.dalike > 0 }">
								<small>[ 추천수 : ${l.dalike } ]</small>
								</c:if>
						</a></li>
						<li>${l.name }</li>
						<li>${l.dadate }</li>
		...
		<c:otherwise>출력할 글이 없습니다.</c:otherwise>
			...
			<div id="paging">
				<!-- 이동할 때 마다 페이지명을 변수처리 -->
				<c:set var="pageName" value="dailyInfoIdentify" scope="request" />
