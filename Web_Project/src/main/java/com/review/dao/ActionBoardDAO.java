...
public class ActionBoardDAO {
	...
	public ArrayList<HashMap<String, Object>> search(String search, String searchInput, int page) {
		ArrayList<HashMap<String, Object>> list = null;
		Connection conn = DBConnection.dbConn();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT (SELECT count(*) FROM Actionview WHERE " + search
				...
				+ " WHERE "+search+" LIKE CONCAT('%',?,'%') ORDER BY ano DESC LIMIT ?, 5";
